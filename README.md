...
Упрощенная схема отношений между сушностями:

![схема](./resource/Radio.PNG)
...
3) Также имеется свойство "категория или жанр стоанции": "разговорный", "поп", "рок", "юмор" и так далее.  Жанры и "станции"
   ...
   Общий список станций (выборка из БД) должен формироваться в соответсвии с запросоми(варианты): 1) "Дай мне список 
   работающих станций с указанным набором языков, жанров, и битрейтами не хуже заданного 2) Дай мне список работающих 
   станций с указанным набором языков, жанров 3) м так далее (другие запросы).  В данном примере эти запрсоы не реализованы.
...
4) Модуля "бизнес-логики" в данном примере нет.  В некоторых описаниях "чистой архитектуры" этот "слой" называют 
   "use cases"
...
Для маппинга используется https://mapstruct.org/. Возможны вариации с extension Kotlin и др. Но mapstruct реально сильно
упрощает описание маппингов при минимальных усилиях.

"Наследование" структур БД: приоритет @MappedSuperclass, пример StationDb.
Реализация отношения HAS-A: приоритет @Embeddable и @Embedded, пример Base и StationDb.
